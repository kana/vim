...
+ (void)installMenuItem:(id)sender
{
	if(NSMenu* editMenu = [self findEditMenu])
	{
		...
		id <NSMenuItem> menuItem = [editMenu addItemWithTitle:[NSString stringWithFormat:NSLocalizedString(@"Edit in %@%@", nil), ODBEditorName, ellips] action:@selector(editInODBEditor:) keyEquivalent:@"E"];
